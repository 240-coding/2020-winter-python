file = open("contact.txt", "r", encoding="UTF8")   # 연락처 파일 열기
contact_list = []
...
def printList():   # 전체 연락처 출력
    ...
    for i in range(len(contact_list)):
        print(contact_list[i][0], contact_list[i][1])   # 이름과 전화번호 순서대로 출력
    print("\n")  # 연락처 전체 출력 후 줄바꿈


def addContact(name, number):  # 연락처 추가
    contact_list.append([name, number])  # 새 연락처를 리스트에 추가함
    contact_list.sort()  # 리스트 재정렬
    print("\n")
    printList()  # 재정렬된 리스트 출력
...
while True:
    ...
    4 : 연락처 수정
    5 : 종료
    메뉴를 선택하세요[1-5] : """))  # 사용자로부터 메뉴 번호 입력 받음
    print("\n")
